...
generalized C3 leaf-photosynthesis model, that operates in steady-state.
Therefore, it is not an ODE model and is made from two functions, a solving
function and the function, calling the model. The model function takes, so
called environmental variables as input. They are Light, Temperature, CO2 and
O2 concentration. Based on those input, the outputs of the model are
calculated. For the publication the most important outputs are Linear and
Cyclic Electron Flow, NPQ and Net carbon assimilation. The goal of the model is
to investigate how photosynthesis is mainly controlled in steady-state
conditions. It is built around the idea that control is shared between CBB and
b6f, but with a special focus on b6f as the main regulatory bottleneck. For the
analysis, the simulations are mainly performed under light intensities from
0-2400 and an additional perturbation of a model parameter to investigate
scenarios of cytb6f and rubisco limitation.

The reproduction of the model was straightforward, but their github only
included two example simulation, which had no connection to the respective
publication. It is not disclosed how the model perturbations are performed and
one has to derive it from the publication, which for some of the cases can be
difficult as the model to publication connection is not clear from time to
time.
"""
...
def get_johnson2021(
    ...
    if case_id == 5:
        ...
        Kn2_a = (
            (np.sqrt(num_kn) - Kp2 * phi2P_a + Ku2 * phi2P_a + Kp2 * q2_a)
            / (2 * phi2P_a)
            - Kf
            - Ku2
            - Kd

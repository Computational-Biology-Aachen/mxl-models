...
def lumen_proton_flux_atp(ATPConductivity, pmf, thresholdpmf, active_atps,
                           leakConductivity, leakpmf, lumenVolumePerArea):
    flux = proton_flux_atp(ATPConductivity, pmf, thresholdpmf, active_atps)
    leak = proton_flux_leak(leakConductivity, pmf, leakpmf)
    return -(flux + leak) / lumenVolumePerArea


def atp_production(ATPConductivity, pmf, thresholdpmf, active_atps,
                   Na, LumenVolume, lumenVolumePerArea):
    flux = proton_flux_atp(ATPConductivity, pmf, thresholdpmf, active_atps)
    vscale = Na * LumenVolume / lumenVolumePerArea
    return flux * (3.0 / 12.0) * vscale


def r_atps_activate(kATPsActivate, fdx_r, active_atps):
    return kATPsActivate * fdx_r * (1.0 - active_atps)


def r_atps_inactivate(kATPsInactivate, active_atps):
    return kATPsInactivate * active_atps
...
def build_zaks_model() -> Model:
    ...
    # PQ pool fractions
    m.add_derived("PQfrac", lambda pq, pool: pq / pool, args=["PQ", "QuinonePoolSize"])
    m.add_derived("PQH2frac", lambda pqh2, pool: pqh2 / pool, args=["PQH2", "QuinonePoolSize"])
    m.add_derived("QBempty", lambda qb_n, qb_r1, qb_r2: 1.0 - qb_n - qb_r1 - qb_r2,
                  args=["QBneut", "QBred1", "QBred2"])
    ...
    m.add_reaction(
        "r_q2",
        fn=lambda kETQAtoQB1, QAred, QBneut, esq:
            kETQAtoQB1 * QAred * QBneut * esq,
    ...
    m.add_reaction(
        "r_q3",
        fn=lambda kETQAtoQB2, QAred, QBred1, esq:
            kETQAtoQB2 * QAred * QBred1 * esq,
    ...
    m.add_reaction(
        "r_q4",
        fn=lambda kETQB1toQA, QAox, QBred1:
            kETQB1toQA * QAox * QBred1,
    ...
    m.add_reaction(
        "r_q5",
        fn=lambda kETQB2toQA, QAox, QBred2:
            kETQB2toQA * QAox * QBred2,
    ...
    m.add_reaction(
        "psi_1",
        fn=lambda kETPCP700, PCr, P700ox:
            max(kETPCP700 * PCr * P700ox, 0.0),
    ...
    m.add_reaction(
        "psi_2",
        fn=lambda light, PSIcrossSection, kETP700Fdx, P700r, Fdxox:
            max(light * PSIcrossSection * kETP700Fdx * P700r * Fdxox, 0.0),
    ...
    m.add_reaction(
        "atps_activate",
        fn=r_atps_activate,
        args=["kATPsActivate", "Fdxr", "ActiveATPs"],
        stoichiometry={"ActiveATPs": 1},
    )
    m.add_reaction(
        "atps_inactivate",
        fn=r_atps_inactivate,
...
if __name__ == "__main__":
    model = build_zaks_model()
    print("Variables :", list(model.get_variable_names()))
    print("Parameters:", list(model.get_parameter_names()))
    print("Reactions :", list(model.get_reaction_names()))
    print("Derived   :", list(model.get_derived_variable_names()))

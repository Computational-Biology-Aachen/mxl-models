r"""Pfennig 2024 cyanobacteria model.
...
cbb: Calvin-Benson-Bassham cycle ocp: orange carotenoid protein pbs:
phycobilisome psi: photosystem I psii: photosystem II lcf: light conversion
factor pq: plastoquinone pc: plastocyanine fd: ferredoxin Ho: cytoplasmic
protons Hi: lumenal protons CCM: carbon-concentrating mechanism FNR:
Ferredoxin-NADP+ Reductase 2PG: 2-phosphoglycolate 3PGA: 3-phosphoglycerate
ADP: Adenosine diphosphate ATP: Adenosine triphosphate ATPsynth: ATP synthase
COX: Cytochrome c oxidase Cyd: Cytochrome bd quinol oxidase Cyt b6f: Cytochrome
b6f complex Flv 1/3: Flavodiiron protein dimer 1/3 NADP+: Nicotinamide adenine
dinucleotide phosphate NADPH: reduced Nicotinamide adenine dinucleotide
phosphate NAD+: Nicotinamide adenine dinucleotide NADH: reduced Nicotinamide
adenine dinucleotide NDH-1: NAD(P)H Dehydrogenase-like complex 1 NDH-2: NAD(P)H
Dehydrogenase complex 2 Oxy: RuBisCO oxygenation PR: Photorespiration SDH:
...
def light_gaussian_led(
    ...
    r"""Get a range of normalised light intensities (400 nm, 700 nm) describing a gaussean LED.

    - wavelength (float): wavelength with maximal intensity.
    - intensity (float, optional): integrated light intensity. Defaults to 1.
    - spread (float, optional): spread (variance) of the distribution. Defaults to
      10\.

    Returns an ArrayLike of wavelength-specific light intensities.
    """
    x = np.arange(390, 711)
    light = pd.Series(
...
def _get_pigment_association(
    *,
    ...
    r"""Create a DataFrame containing the fractions of pigments associated to the main complexes (PS1, PS2. PBS)

    Args: ps1_ratio (float): ratio of PS1:PS2 beta_carotene_method (str): how to
    calculate the beta carotene energy donation, one of 'original' or
    'stoichiometric' pigments (pd.DataFrame): DataFrame with the pigment amounts
    [mg(pigment) mg(chlorophyll a)^-1] in named columns verbose (bool): issue a
    warning if provided and calculated beta carotene amounts don't match
...
def _absorption_per_complex(
    ...
    r"""Compute spectrally integrated light absorption for PSI, PSII, and PBS.

    Weights wavelength-resolved absorption coefficients by pigment content and
    pigment-to-complex association, then integrates over the incident light
    spectrum. Returns absorption values in \[mg(Chla) mmol(Chl)^-1 * (absorption
    unit)\] for each complex as (ps1, ps2, pbs).
    """
...
def _p_hlumen(
    ...
    r"""Convert lumenal proton concentration [mmol mol(Chl)^-1] to pH using the lumen conversion factor."""
...
def _p_hcytoplasm(
    ...
    r"""Convert cytoplasmic proton concentration [mmol mol(Chl)^-1] to pH using the cytoplasm conversion factor."""
...
def _atp_synthase(
    ...
    r"""Rate of ATP synthesis driven by the transmembrane proton gradient (ΔpH).
...
def _stoich_hi_at_psynthase(
    ...
    r"""Stoichiometric factor for Hi: HPR lumenal protons consumed per ATP synthesized, scaled by lumen buffering capacity."""
    return -hpr / b_hi
...
def _stoich_ho_at_psynthase(
    ...
    r"""Stoichiometric factor for Ho: HPR cytoplasmic protons released per ATP synthesized, scaled by cytoplasm buffering capacity."""
    return hpr / b_ho
...
def _stoich_ho_v_nad_hconsumption(
    ...
    r"""Stoichiometric factor for Ho: 1 cytoplasmic proton consumed per NADH oxidised, scaled by cytoplasm buffering capacity."""
    return -1 / b_ho
...
def _atp_consumption(
    ...
    r"""Lumped first-order ATP consumption rate representing cellular maintenance and biosynthesis demand [mmol mol(Chl)^-1 s^-1]."""
...
def _nadph_consumption(
    ...
    r"""Lumped NADH consumption rate for cytoplasmic biosynthetic reactions, proportional to NADH and cytoplasmic proton concentration \[mmol mol(Chl)^-1 s^-1\]."""
...
def _d_g_p_h(
    ...
    r"""Proton free energy coefficient RT·ln(10) [kJ mol^-1], used to convert pH differences to Gibbs free energy contributions."""
...
def _ps_ratio(
    ...
    r"""Molar PSI:PSII ratio corrected for subunit stoichiometry (3 Chl per PSI monomer vs 2 per PSII monomer)."""
...
def _molar_hlumen(
    ...
    r"""Calculate the molar concentration of protons in the thylakoid lumen from the chlorophyll-normalised concentration.
...
def _molar_hcytoplasm(
    ...
    r"""Calculate the molar concentration of protons in the cytoplasm from the chlorophyll-normalised concentration.
...
def _p_h(
    ...
    r"""Compute pH from molar proton concentration [mol l^-1]."""
...
def _deltap_h(
    ...
    r"""Transmembrane ΔpH = pHcytoplasm - pHlumen; positive when lumen is more acidic than cytoplasm."""
...
def _ps_normabsorption_tot(
    ...
    r"""Sum growth-light and measuring-light absorption for PSI and PSII to give total absorbed excitation per photosystem."""
...
def _moiety_1(
    ...
    r"""Compute the complementary pool of a conserved moiety: total - part (e.g. PQ_red = PQ_tot - PQ_ox)."""
    return total - part
...
def _keq_p_qred(
    ...
    r"""pH-dependent equilibrium constant for PQ reduction by the PSII primary acceptor QA."""
    DG1 = -e0_qa * f
...
def _ps2states(
    ...
    r"""Analytical quasi-steady-state solution for the four PSII state occupancies.

    Returns (B0, B1, B2, B3) where B0 = dark-relaxed open PSII, B1 =
    charge-separated (closed) PSII with oxidised PQ, B2 = open PSII with reduced
    PQ, B3 = closed PSII with reduced PQ. State-transition quenching shifts PSIIq
    from the unquenched pool. Units: [mmol mol(Chl)^-1].
    """
...
def _psii(
    ...
    r"""PSII photochemical rate: electron transfer from water to PQ via charge-separated state B1 [mmol mol(Chl)^-1 s^-1].

    Scaled by 0.5 because k2 is defined per single-electron step (B1→B2) but the
    full PSII reaction extracts 2 electrons per PQ reduction.
    """
    return (
        0.5 * k2 * b1
...
def _keq_v_sdh(
    ...
    r"""pH-dependent equilibrium constant for PQ reduction by succinate dehydrogenase (SDH)."""
...
def _v_respiration(
    ...
    r"""Approximation of respiration resulting in the consumption of 3PGA with generation of ATP, NADPH, NADH, and succinate (from fumarate)"""
...
def _vbd(
    ...
    r"""Electron flow from PQ to O2 via bd-type (Cyd) terminal oxidase, consuming PQH2 and producing water [mmol mol(Chl)^-1 s^-1]."""
...
def _vb6f(
    ...
    r"""Electron flow from PQH2 to PC via Cyt b6f complex, pumping protons into the lumen via the Q-cycle [mmol mol(Chl)^-1 s^-1]."""
...
def _vaa(
    ...
    r"""Electron flow from PC to O2 via aa3-type cytochrome c oxidase (COX), consuming O2 and pumping protons [mmol mol(Chl)^-1 s^-1]."""
...
def _keq_fa_fd(
    ...
    r"""Equilibrium constant for electron transfer from PSI iron-sulfur cluster FA to free ferredoxin (Fd)."""
    DG1 = -e0_fa * f
    DG2 = -e0_fd * f
    DG = -DG1 + DG2
    return np.exp(-DG / rt)
...
def _keq_pcp700(
    ...
    r"""Equilibrium constant for electron transfer from reduced PC to the oxidised PSI reaction centre P700+."""
    DG1 = -e0_pc * f
    DG2 = -e0_p700 * f
    DG = -DG1 + DG2
    return np.exp(-DG / rt)
...
def _calc_a(
    ...
    r"""Intermediate variable a in the analytical QSS solution for PSI states: back-transfer rate from FA to Fd minus light rate."""
...
def _calc_b(
    ...
    r"""Intermediate variable b in the analytical QSS solution for PSI states: sum of Fd reduction, fluorescence decay, and light absorption rate constants."""
...
def _calc_c(
    ...
    r"""Intermediate variable c in the analytical QSS solution for PSI states: reverse PC-to-P700 transfer rate plus light rate."""
...
def _calc_d(
    ...
    r"""Intermediate variable d in the analytical QSS solution for PSI states: combined PC reduction and fluorescence flux."""
...
def _calc_f(
    ...
    r"""Intermediate variable f in the analytical QSS solution for PSI states: normalisation factor for PSI state distribution."""
    return (1 + a / b) / d
...
def _calc_y0(
    ...
    r"""QSS concentration of open (ground-state) PSI \[mmol mol(Chl)^-1\]: P700 reduced, FA oxidised, able to absorb light."""
...
def _calc_y2(
    ...
    r"""QSS concentration of doubly-reduced PSI state Y2 \[mmol mol(Chl)^-1\]: P700 reduced, FA reduced."""
...
def _calc_y1(
    ...
    r"""QSS concentration of charge-separated PSI state Y1 \[mmol mol(Chl)^-1\]: P700 oxidised, FA reduced."""
...
def _ps1_states(
    ...
    r"""Analytical quasi-steady-state solution for PSI state occupancies.

    Returns (Y0, Y1, Y2) where Y0 = open PSI (P700 reduced, FA oxidised), Y1 =
    charge-separated PSI (P700 oxidised, FA reduced), and Y2 = doubly-reduced PSI
    (P700 reduced, FA reduced). Units: [mmol mol(Chl)^-1].
    """
    k_l1 = light_ps1
...
def _v_ps1(
    ...
    r"""Net PSI photochemical rate: light absorption by open PSI (Y0) minus back-reaction from charge-separated state (Y1) [mmol mol(Chl)^-1 s^-1]."""
...
def _fluorescence_ps2(
    ...
    r"""PSII fluorescence signal: difference between total-light and growth-light-only open PSII states, weighted by fluorescence rate constant kF."""
...
def _fluorescence_ps1(
    ...
    r"""PSI fluorescence signal: difference in charge-separated PSI state Y1 between total-light and growth-light, weighted by k_F1."""
...
def _fluorescence_tot(
    ...
    r"""Total chlorophyll fluorescence as the sum of PSII, PSI, and PBS contributions."""
...
def _d_g0_fnr(
    ...
    r"""Standard Gibbs free energy [kJ mol^-1] for the FNR reaction: 2 Fd_red + NADP+ → 2 Fd_ox + NADPH + H+."""
    DG1 = -e0_fd * f
...
def _d_g_fnr(
    ...
    r"""Actual Gibbs free energy [kJ mol^-1] of the FNR reaction at current metabolite concentrations."""
    # Caclulate delta G
    s = fd_red**2 * nadp
    P = fd_ox**2 * nadph
...
def _v_fnr(
    ...
    r"""Rate of NADPH production by FNR, thermodynamically gated.

    Forward direction (ΔG < 0): Fd_red reduces NADP+ to NADPH. Reverse direction
    (ΔG > 0): NADPH reduces Fd_ox, active in darkness. Units: \[mmol mol(Chl)^-1
    s^-1\].
    """
...
def _v_o2out(
    ...
    r"""Exchange of oxygen with the environment.
...
def _stoich_ho_v_ps2(
    ...
    r"""Stoichiometric factor for Ho: 2 cytoplasmic protons consumed per PSII turnover (water splitting), scaled by cytoplasm buffering capacity."""
    return -2 / b_ho
...
def _stoich_hi_v_ps2(
    ...
    r"""Stoichiometric factor for Hi: 2 lumenal protons released per PSII turnover (water splitting), scaled by lumen buffering capacity."""
    return 2 / b_hi
...
def _stoich_ho_v_respiration(
    ...
    r"""Stoichiometric factor for Ho: ~4.9 cytoplasmic protons produced per respiratory turnover, scaled by cytoplasm buffering capacity."""
    return 4.926 / b_ho
...
def _stoich_ho_vbd(
    ...
    r"""Stoichiometric factor for Ho: 4 cytoplasmic protons consumed per O2 reduced by bd-type oxidase (Cyd), scaled by cytoplasm buffering capacity."""
    return -4 / b_ho
...
def _stoich_hi_vbd(
    ...
    r"""Stoichiometric factor for Hi: 4 lumenal protons released per O2 reduced by bd-type oxidase (Cyd), scaled by lumen buffering capacity."""
    return 4 / b_hi
...
def _stoich_hi_vb6f(
    ...
    r"""Stoichiometric factor for Hi: 4 lumenal protons released per Cyt b6f turnover (Q-cycle), scaled by lumen buffering capacity."""
    return 4 / b_hi
...
def _stoich_ho_vb6f(
    ...
    r"""Stoichiometric factor for Ho: 2 cytoplasmic protons consumed per Cyt b6f turnover (Q-cycle), scaled by cytoplasm buffering capacity."""
    return -2 / b_ho
...
def _stoich_hi_vaa(
    ...
    r"""Stoichiometric factor for Hi: 1 lumenal proton released per aa3-COX turnover, scaled by lumen buffering capacity."""
    return 1 / b_hi
...
def _stoich_ho_vaa(
    ...
    r"""Stoichiometric factor for Ho: 5 cytoplasmic protons consumed per aa3-COX turnover, scaled by cytoplasm buffering capacity."""
    return -5 / b_ho
...
def _stoich_ho_v_fnr(
    ...
    r"""Stoichiometric factor for Ho: 1 cytoplasmic proton consumed per FNR turnover (NADPH production), scaled by cytoplasm buffering capacity."""
    return -1 / b_ho
...
def _stoich_hi_v_pass(
    ...
    r"""Stoichiometric factor for Hi: 1 lumenal proton lost per passive leakage event, scaled by lumen buffering capacity."""
    return -1 / b_hi
...
def _stoich_ho_v_pass(
    ...
    r"""Stoichiometric factor for Ho: 1 cytoplasmic proton gained per passive leakage event, scaled by cytoplasm buffering capacity."""
    return 1 / b_ho
...
def _stoich_ho_v_ndh(
    ...
    r"""Stoichiometric factor for Ho: 1 cytoplasmic proton consumed per NDH-2 turnover (non-electrogenic PQ reduction), scaled by cytoplasm buffering capacity."""
    return -1 / b_ho
...
def _lumped_photorespiration(
    ...
    r"""Calculate the rate of PG recycling"""
...
def _stoich_ho_v_p_rsalv(
    ...
    r"""Stoichiometric factor for Ho: 1 cytoplasmic proton released per photorespiratory salvage turnover, scaled by cytoplasm buffering capacity."""
    return 1 / b_ho
...
def _cbb_cycle(
    ...
    r"""Rate of CO2 fixation by the CBB cycle [mmol mol(Chl)^-1 s^-1], limited by energy (ATP/NADPH), gas (CO2/O2), and light-activated enzyme fraction."""
...
def _rubisco_oxygenation(
    ...
    r"""Rate of RuBisCO oxygenation producing 2-phosphoglycolate (PG) \[mmol mol(Chl)^-1 s^-1\], limited by carbon (3PGA), energy, gas (O2/CO2), and light activation."""
...
def _cbb_energy_mm(
    ...
    r"""Michaelis-Menten energy-limitation factor for CBB and Oxy: fraction of maximal rate given current ATP and NADPH concentrations."""
...
def _cbb_gas_mm_o2(
    ...
    r"""Michaelis-Menten CO2 saturation factor for CBB with competitive O2 inhibition of RuBisCO carboxylation."""
...
def _oxy_gas_mm_co2(
    ...
    r"""Michaelis-Menten O2 saturation factor for RuBisCO oxygenation with competitive CO2 inhibition."""
...
def _oxy_carbon_mm(
    ...
    r"""Michaelis-Menten carbon-substrate factor for RuBisCO oxygenation: 3PGA saturation representing carbon pool availability."""
...
def _stoich_ho_v_cbb(
    ...
    r"""Stoichiometric factor for Ho: 5 cytoplasmic protons consumed per CBB cycle turnover, scaled by cytoplasm buffering capacity."""
    return -5 / b_ho
...
def _stoich_ho_v_oxy(
    ...
    r"""Stoichiometric factor for Ho: 5 cytoplasmic protons consumed per RuBisCO oxygenation turnover, scaled by cytoplasm buffering capacity."""
    return -5 / b_ho
...
def _cbb_activation(
    ...
    r"""Rate of CBB enzyme activation by reduced ferredoxin (Fd_red) via the ferredoxin-thioredoxin regulatory pathway [s^-1].

    CBBa approaches Fd_red / (KMFdred + Fd_red) at steady state, linking carbon
    fixation capacity to the redox state of the photosynthetic electron transport
    chain.
    """
...
def _v_flv_v2(
    ...
    r"""Electron flow from Fd to O2 via Flv1/3-Heterodimere"""
    return (
        k_o2
        * o2
        * ho
...
def _co2p_k1(
    ...
    r"""First dissociation constant (pK1) of carbonic acid as a function of temperature T [K] and salinity S [unitless] (MojicaPrieto2002)."""
    return (
...
def _co2_k_henry(
    ...
    r"""Henry's law solubility constant for CO2 [mol l^-1 atm^-1] as a function of temperature T [K] and salinity S [unitless] (König2019)."""
    return np.exp(
        -58.0931
        + 90.5069 * 100 / t
...
def _co2sol(
    ...
    r"""Dissolved CO2 concentration [mol l^-1] at partial pressure CO2pp [atm], temperature T [K], and salinity S, via Henry's law."""
...
def _co2aq(
    ...
    r"""Aqueous CO2 (CO2·H2O) fraction of total dissolved inorganic carbon [mol l^-1], pH-dependent via the first dissociation equilibrium."""
...
def _v_ccm_v2(
    ...
    r"""Exchange of CO2 with the environment.

    Viewed in inward direction. raises the total cellular CO2 concentration to the
    maximum allowed by external concentration or solubility assumes a constant
...
def _stoich_ho_v_flv_hill(
    ...
    r"""Stoichiometric factor for Ho: 4 cytoplasmic protons consumed per O2 reduced by Flv 1/3, scaled by cytoplasm buffering capacity."""
    return -4 / b_ho
...
def _stoich_hi_v_nq_mm(
    ...
    r"""Stoichiometric factor for Hi: 1 lumenal proton released per NDH-1 turnover (cyclic electron flow), scaled by lumen buffering capacity."""
    return 1 / b_hi
...
def _stoich_ho_v_nq_mm(
    ...
    r"""Stoichiometric factor for Ho: 3 cytoplasmic protons consumed per NDH-1 turnover (cyclic electron flow), scaled by cytoplasm buffering capacity."""
    return -3 / b_ho
...
def _v_nq_mm(
    ...
    r"""Rate of PQ reduction by NDH-1 complex using Fd_red as electron donor; Michaelis-Menten kinetics for both PQ_ox and Fd_red [mmol mol(Chl)^-1 s^-1]."""
...
def _free_pbs(
    ...
    r"""Fraction of free phycobilisomes"""
...
def _ocp_activation(
    ...
    r"""Activation of orange carotenoid protein.
...
def _fluorescence_pbs_ocp(
    ...
    r"""Fluorescence from free PBS, reduced by active OCP quenching; proportional to unquenched fraction (1 - OCP)."""
...
def _ps_normabsorption_ocp(
    ...
    r"""Normalised light absorption per PSI and PSII molecule, including PBS antenna energy transfer attenuated by OCP quenching."""
...
def _psii_unquench(
    ...
    r"""Rate of PSII recovery from the quenched state (PSIIq → PSII), inhibited by reduced PQ via a Hill-type term."""
    nUnquench = 1
    return (
...
def add_atpase(
    ...
    r"""Add the ATP synthase reaction (vATPsynthase) to the model, driven by the transmembrane proton gradient."""
    m.add_reaction(
        "vATPsynthase",
        fn=_atp_synthase,
        args=[
...
def add_consuming_reactions(
    ...
    r"""Add lumped ATP and NADH consumption reactions representing cellular maintenance and biosynthesis demand."""
    m.add_reaction(
        "vATPconsumption",
        fn=_atp_consumption,
        args=[
...
def add_electron_transport_chain(
    ...
    r"""Add the complete photosynthetic electron transport chain, proton handling, and associated derived quantities to the model.

    Includes derived variables (pH, equilibrium constants, moieties), QSS
    surrogates for photosystem states, and all electron-transfer reactions from
    water splitting through to Fd reduction, O2 exchange, and CO2 uptake via the
    CCM.
    """
    m.add_derived(
        "RT",
...
def add_photorespiratory_salvage(
    ...
    r"""Adds the lumped reaction recycling (2-phospho)glycolate from photorespiration.

    The standard stoichiometry assumes complete recycling via glycolate
    dehydrogenase and the glycerate pathway (via tartronic semialdehyde):

    2 PG + (NADPH + Ho) + 2 NAD + ATP --> (3PGA + CO2) + NADP + (2 NADH + 2 Ho) +
    ADP (+ Pi)
    """
    m.add_reaction(
        "vPRsalv",
...
def add_cbb_and_oxy(
    ...
    r"""Add CBB cycle, RuBisCO oxygenation, and Fd-dependent CBB activation reactions to the model."""
...
def get_pfennig2024_synechocystis(
    ...
    r"""Construct and return the complete Synechocystis photosynthesis ODE model.
